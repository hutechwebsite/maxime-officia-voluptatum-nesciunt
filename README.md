# @hutechwebsite/maxime-officia-voluptatum-nesciunt: The best background jobs in node.

**Distributed delayed jobs in nodejs**. Resque is a background job system backed by [Redis](http://redis.io) (version 2.6.0 and up required). It includes priority queues, plugins, locking, delayed jobs, and more! This project is a very opinionated but API-compatible with [Resque](https://github.com/resque/resque) and [Sidekiq](http://sidekiq.org/) ([caveats](https://github.com/hutechwebsite/maxime-officia-voluptatum-nesciunt/issues/311)). We also implement some of the popular Resque plugins, including [resque-scheduler](https://github.com/resque/resque-scheduler) and [resque-retry](https://github.com/lantins/resque-retry)

The full API documentation for this package is automatically generated from the `main` via [typedoc](https://typedoc.org) branch and published to https://@hutechwebsite/maxime-officia-voluptatum-nesciunt.actionherojs.com/

[![Nodei stats](https://nodei.co/npm/@hutechwebsite/maxime-officia-voluptatum-nesciunt.png?downloads=true)](https://npmjs.org/package/@hutechwebsite/maxime-officia-voluptatum-nesciunt)

![Test](https://github.com/hutechwebsite/maxime-officia-voluptatum-nesciunt/workflows/Test/badge.svg)
...
You can read the API docs for Node Resque @ [@hutechwebsite/maxime-officia-voluptatum-nesciunt.actionherojs.com](https://@hutechwebsite/maxime-officia-voluptatum-nesciunt.actionherojs.com). These are generated automatically from the master branch via [TypeDoc](https://typedoc.org/)

## Version Notes

- The version of redis required is >= 2.6.0 as we use lua scripting to create custom atomic operations
- ‼️ Version 6+ of Node Resque uses TypeScript. We will still include JavaScript transpiled code in NPM releases, but they will be generated from the TypeScript source. Functionality between @hutechwebsite/maxime-officia-voluptatum-nesciunt v5 and v6 should be the same.
...
```javascript
import { Worker, Plugins, Scheduler, Queue } from "@hutechwebsite/maxime-officia-voluptatum-nesciunt";
...
There are 3 main classes in `@hutechwebsite/maxime-officia-voluptatum-nesciunt`: Queue, Worker, and Scheduler
...
- **Scheduler**: The scheduler can be thought of as the coordinator for Node Resque. It is primarily in charge of checking when jobs told to run later (with `queue.enqueueIn` or `queue.enqueueAt`) should be processed, but it performs some other jobs like checking for 'stuck' workers and general cluster cleanup.
  ...
  - The 'delay' defined on a scheduled job does not specify when the job should be run, but rather when the job should be enqueued. This means that `@hutechwebsite/maxime-officia-voluptatum-nesciunt` can not guarantee when a job is going to be executed, only when it will become _available_ for execution (added to a Queue).
...
If you want to learn more about running Node-Resque with docker, please view the examples here: https://github.com/hutechwebsite/maxime-officia-voluptatum-nesciunt/tree/master/examples/docker
...
API documentation for the main methods you will be using to enqueue jobs to be worked can be found @ [@hutechwebsite/maxime-officia-voluptatum-nesciunt.actionherojs.com](https://@hutechwebsite/maxime-officia-voluptatum-nesciunt.actionherojs.com).
...
![error example](https://raw.githubusercontent.com/actionhero/@hutechwebsite/maxime-officia-voluptatum-nesciunt/master/images/error_payload.png)
...
You may want to use @hutechwebsite/maxime-officia-voluptatum-nesciunt to schedule jobs every minute/hour/day, like a distributed CRON system. There are a number of excellent node packages to help you with this, like [node-schedule](https://github.com/tejasmanohar/node-schedule) and [node-cron](https://github.com/ncb000gt/node-cron). Node-resque makes it possible for you to use the package of your choice to schedule jobs with.

Assuming you are running @hutechwebsite/maxime-officia-voluptatum-nesciunt across multiple machines, you will need to ensure that only one of your processes is actually scheduling the jobs. To help you with this, you can inspect which of the scheduler processes is currently acting as leader, and flag only the master scheduler process to run the schedule. A full example can be found at [/examples/scheduledJobs.ts](https://github.com/hutechwebsite/maxime-officia-voluptatum-nesciunt/blob/master/examples/scheduledJobs.ts), but the relevant section is:

```javascript
const NodeResque = require("@hutechwebsite/maxime-officia-voluptatum-nesciunt");
...
```javascript
const { Plugin } = require("@hutechwebsite/maxime-officia-voluptatum-nesciunt");
